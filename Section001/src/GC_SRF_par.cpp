...
int main(int argc, char *argv[]) {
  ...
  float win_len;
  for (incnum = rank * 4; incnum <= 60; incnum = incnum + comm_size * 4) {
    for (win_len = 5; win_len <= 100; win_len = win_len + 5) {
      ...
      // WIN_LEN="<<win_len<<"    OK!"<<endl; cout<<"Now for: INCI="<<incnum<<"
      // WIN_LEN="<<win_len<<" OK!"<<endl; mt_rot2( data_Z, data_R,
      // npts_Z,mdir_angle_zr2lq, data_L, data_Q );

      if (detial_flag == "detail_info") {
      ...
      // cout<<"aaa Now for: INCI="<<incnum<<" WIN_LEN="<<win_len<<" OK!"<<endl;
      sl_name =
          myFormatStringByFun((char *)"rf.%02d.%03.f.sl.sac", incnum, win_len);
      sq_name =
          myFormatStringByFun((char *)"rf.%02d.%03.f.sq.sac", incnum, win_len);
      st_name =
          myFormatStringByFun((char *)"rf.%02d.%03.f.st.sac", incnum, win_len);
      nsl_name =
          myFormatStringByFun((char *)"rf.%02d.%03.f.nsl.sac", incnum, win_len);
      nsq_name =
          myFormatStringByFun((char *)"rf.%02d.%03.f.nsq.sac", incnum, win_len);
      nst_name =
          myFormatStringByFun((char *)"rf.%02d.%03.f.nst.sac", incnum, win_len);
      // cout<<"bbb Now for: INCI="<<incnum<<" WIN_LEN="<<win_len<<" OK!"<<endl;
      // cout<<"ccc Now for:"<<sl_name<<" OK!"<<endl;
